...
class Server:
    ...
    def analyze_client_state(self, client_state):
        ...
        if client_state.location.HasField("front_track_id"):
            ...
            if self.railway.map.has_bad_track_condition(
                client_state.location.front_track_id
            ):
        ...
        if proxy_resp.HasField("is_master"):
            if proxy_resp.is_master:
                ...
                if not self.connected_to_master:
                    ...
                    threading.Thread(
                        target=self.listen_for_master, args=(self.host, 4444)
                    ).start()
    ...
    def master_proxy_communication(self, sock, data):
        ...
        # CHECK FOR HEARTBEAT HERE
            ...
            LOGGER.warning(
                f"Server received msg from proxy with missing co0ntent: {proxy_resp}"
            )
    ...
    def handle_client_state(self, client_state):
        ...
        # update train location
        self.railway.update_train(
            train, TrainState(client_state.train.state), client_state.location
        )
    ...
    def handle_connection(self, conn):
        ...
        if data:
            init_message = (
                TrackNet_pb2.InitConnection()
            )  # Assuming this is your wrapper message
            init_message.ParseFromString(data)
            LOGGER.debug(f"Received: {init_message}")
