...
class Railway:
    ...
    def update_train(
        ...
        train,
        state,
        location_msg: TrackNet_pb2.Location,
        route_msg: TrackNet_pb2.Route,
    ):
        # check if new track
        if (
            train.location.front_cart["track"] is None
            or train.location.front_cart["track"].name != location_msg.front_track_id
        ):
            # add to new track
